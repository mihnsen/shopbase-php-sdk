...
{
    const SHOPBASE_API_KEY_NAME = 'SHOPBASE_API_KEY';
    const SHOPBASE_API_SECRET_NAME = 'SHOPBASE_API_SECRET';
    ...
    {
        $args = array();
        if (!is_null($this->onshopbase_domain)) {
            $args['base_uri'] = sprintf(
                "https://%s/admin/api/%s/",
                $this->onshopbase_domain,
                $this->getApiVersion()
            );
        }
        if (!is_null($this->access_token)) {
            $args['headers']['X-Shopbase-Access-Token'] = $this->access_token;
        }
    ...
    {
        $this->api_key = getenv(self::SHOPBASE_API_KEY_NAME);
    }

    public function loadApiSecretFromEnv()
    {
        $this->api_secret = getenv(self::SHOPBASE_API_SECRET_NAME);
    }
}
